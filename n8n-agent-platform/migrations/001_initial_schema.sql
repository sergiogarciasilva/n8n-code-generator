-- Initial schema for n8n Agent Platform
-- This creates the base tables required for the platform

-- Organizations table (needed for foreign keys)
...
COMMENT ON TABLE workflows IS 'n8n workflows registered in the platform';
COMMENT ON TABLE agents IS 'AI agents for workflow optimization';
COMMENT ON TABLE workflow_analyses IS 'Historical analysis results';
COMMENT ON TABLE optimization_suggestions IS 'AI-generated optimization suggestions';
COMMENT ON TABLE workflow_changes IS 'Log of changes made to workflows';
COMMENT ON TABLE agent_executions IS 'Agent execution history and metrics';
COMMENT ON TABLE agent_metrics IS 'Detailed agent performance metrics';
COMMENT ON TABLE workflow_execution_results IS 'Workflow test and execution results';
COMMENT ON TABLE performance_benchmarks IS 'Performance comparison data';
COMMENT ON TABLE agent_schedules IS 'Agent scheduling configuration';
COMMENT ON TABLE system_events IS 'System-wide events and logging';